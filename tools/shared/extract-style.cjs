// 사용법: node extract-style.cjs poistylefunc.js > style_data.json
//
...
/* eslint-disable @typescript-eslint/no-require-imports -- 격리된 CJS eval 샌드박스 스크립트 */
const fs = require("fs");
const path = require("path");

const inputPath = process.argv[2];
if (!inputPath) {
  console.error("사용법: node extract-style.cjs <원본JS파일경로>");
  process.exit(1);
}
